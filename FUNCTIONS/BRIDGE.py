...
def find_splitting_transition_set(parent_name, first_stage_module, second_stage_module, relation_graph, pcc_matrix, pcc_theshold):
...
def find_all_splitting_transition_set(first_stage_module, second_stage_module, relation_graph, pcc_matrix, pcc_theshold):
    all_nodes = list()
    for parent_name in first_stage_module:
        if parent_name in relation_graph.vs["name"] and relation_graph.degree(parent_name) > 1:
            nodes = find_splitting_transition_set(parent_name=parent_name, first_stage_module=first_stage_module,
...
def find_all_transition_set(first_stage_module, second_stage_module, relation_graph, first_pcc_matrix, second_pcc_matrix, pcc_theshold):
    all_nodes_split = find_all_splitting_transition_set(
    ...
    all_nodes_merge = find_all_splitting_transition_set(
    ...
    with open(f'transition_set_{first_pcc_matrix.name.split("_")[-1]}_vs_{second_pcc_matrix.name.split("_")[-1]}.txt', 'w') as file:
...
def find_targets(DEG, transition_set):
    targets = list()
    for i in DEG:
