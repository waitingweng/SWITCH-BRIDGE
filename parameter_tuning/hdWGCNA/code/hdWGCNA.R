...
setwd("/home/rstudio/R/parameter_tuning/hdWGCNA/Cell")
seurat_obj <- readRDS('./data/discrete_cancer_stages_annotated_hdWGCNA.rds')
...
seurat_obj <- MetacellsByGroups(
  ...
  group.by = c("sample_ID", "discrete_cancer_stage"), # specify the columns in seurat_obj@meta.data to group by
  reduction = 'harmony', # select the dimensionality reduction to perform KNN on
  k = 30, # nearest-neighbors parameter
  # max_shared = 15, # maximum number of shared cells between two metacells
  ident.group = "discrete_cancer_stage" # set the Idents of the metacell seurat object
)
...
seurat_obj <- RunHarmonyMetacells(seurat_obj, group.by.vars='sample_ID')
seurat_obj <- RunUMAPMetacells(seurat_obj, reduction='harmony', dims=1:50)


seurat_obj <- SetDatExpr(
  seurat_obj,
  group_name = "IV", # the name of the group of interest in the group.by column
  group.by='discrete_cancer_stage', # the metadata column containing the cell type info. This same column should have also been used in MetacellsByGroups
...
combined_plot <- wrap_plots(plot_list, ncol = 2) +
  plot_annotation(
    title = "IV Soft Power",
    theme = theme(
      plot.title = element_text(size = 26, hjust = 0.5)  # center & resize
    )
  )
# Display the combined plot
print(combined_plot)
# Optionally, save the combined plot
ggsave(filename = "./figures/soft_powers_IV.svg", plot = combined_plot, width = 12, height = 10)


# run it just for getting TOM
seurat_obj <- ConstructNetwork(
  seurat_obj,
  tom_name = "IV", # Name the TOM file to include deepSplit value
...
write_feather(TOM_df, "./TOM/TOM_IV.feather")
...
for (deepSplit in deepSplit_values) {
  ...
  seurat_obj <- ConstructNetwork(
    seurat_obj,
    tom_name = paste0('4S_deepSplit_', deepSplit), # Name the TOM file to include deepSplit value
    networkType = "unsigned",
    overwrite_tom = TRUE,
  ...
  # Open an SVG device
  svg(filename = paste0("./figures/hdWGCNA_IV_dendrogram_", deepSplit, ".svg"), width = 8, height = 6)
  # Example code if `PlotDendrogram()` returns a ggplot object
  PlotDendrogram(seurat_obj, main = '4S hdWGCNA Dendrogram', 
  ...
  write.csv(modules, file = paste0("./results/module_4S_", deepSplit, ".csv"))
}
